...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption('🐍 贪吃蛇游戏 - 峰哥专属版')
    ...
    def draw_score(self):
        score_text = self.font_medium.render(f"得分: {self.snake.score}", True, WHITE)
        high_score_text = self.font_small.render(f"最高分: {self.high_score}", True, YELLOW)
        speed_text = self.font_small.render(f"速度: {self.snake.speed}", True, WHITE)
        length_text = self.font_small.render(f"长度: {len(self.snake.positions)}", True, WHITE)
    ...
    def draw_game_over(self):
        ...
        score_text = self.font_medium.render(f"最终得分: {self.snake.score}", True, WHITE)
        if self.snake.score > self.high_score:
            self.high_score = self.snake.score
            self.save_high_score()
            new_record_text = self.font_medium.render("🎉 新纪录! 🎉", True, YELLOW)
        ...
        menu_text = self.font_medium.render("按 M 返回菜单", True, WHITE)
        quit_text = self.font_medium.render("按 ESC 退出", True, WHITE)
    ...
    def draw_pause(self):
        ...
        continue_text = self.font_medium.render("按 SPACE 继续", True, WHITE)
        menu_text = self.font_medium.render("按 M 返回菜单", True, WHITE)
    ...
    def draw_menu(self):
        ...
        title_text = self.font_large.render("🐍 贪吃蛇游戏", True, GREEN)
        subtitle_text = self.font_medium.render("峰哥专属版", True, YELLOW)
        
        start_text = self.font_medium.render("按 ENTER 开始游戏", True, WHITE)
        controls_text = self.font_small.render("方向键控制移动 | 空格暂停 | ESC退出", True, LIGHT_GRAY)
        high_score_text = self.font_medium.render(f"历史最高分: {self.high_score}", True, YELLOW)
